...
function display_text(data)
	...
	if data.font==nil then data.font = "WenQuanYi Micro Hei" end
...
function draw_clock_ring(data)
	...
	cairo_select_font_face (cr, "WenQuanYi Micro Hei", CAIRO_FONT_SLANT_NORMAL, txt_weight);
	cairo_set_font_size (cr, txt_size);
	cairo_set_source_rgba (cr, rgb_to_r_g_b(txt_fg_colour, txt_fg_alpha));
...
function draw_gauge_ring(data)
	...
	cairo_select_font_face (cr, "WenQuanYi Micro Hei", CAIRO_FONT_SLANT_NORMAL, caption_weight);
...
function conky_main(color, theme, drawbg, unit, area_code, posfix)
    ...
    settings = {--DAY TEMP
        txt="气温: " .. get_yahoo_weather_info("cur", area_code, unit) .. unitChar,
        x=(w/2)+60               , y=20            ,
        txt_weight=0        , txt_size=12 ,
        txt_fg_colour=fgc , txt_fg_alpha=fga    ,
    };display_text(settings)
    settings = {--DATA
        txt=conky_parse("${time %Y}") .. " " .. conky_parse("${time %b}") .. "" .. conky_parse("${time %d}日"),
        x=(w/2)+60               , y=35            ,
        txt_weight=0        , txt_size=12 ,
        txt_fg_colour=theme , txt_fg_alpha=fga    ,
    };display_text(settings)
	...
	settings = {--NETWORK GRAPH DOWN
		value=tonumber(conky_parse("${downspeedf eth0}")),
		value_max=100              ,
		x=xp                       , y=yp                        ,
		graph_radius=22            ,
		graph_thickness=5          ,
	...
	settings = {--NETWORK GRAPH UP
		value=tonumber(conky_parse("${upspeedf eth0}")),
		value_max=100              ,
		x=xp                       , y=yp                        ,
		graph_radius=17            ,
		graph_thickness=5          ,
